...
\label{sec:Common Models}
\subsection{Tow Level System TLS}
\label{sec:Tow Level System TLS}
\begin{equation}
    H = \varepsilon + \vec{d}\cdot\vec{\sigma}
\end{equation}
If 
$$
\vec{d}=|d|
    \left(\sin(\theta)\cos(\phi),\sin(\theta)\sin(\phi),\cos(\theta)\right)
$$, then, the eigenvalues are $E_\pm = \varepsilon\pm |d|$, with
...
If
$$\vec{d}=(d_1,d_2,d_3) $$, then the eigenvalues are $E_\pm =
\varepsilon\pm |d|$, with normalized eigenvectors (let $d\equiv|d|$):
\begin{equation}
\ket{+} = 
\frac{1}{\sqrt{2d(d_3+d)}}
\begin{pmatrix}
    d_3+d \\ d_1+i d_2
\end{pmatrix},\quad
\ket{-} = 
...
\begin{thebibliography}{1}
    ...
    \bibitem{MordQM} Modern Quantum Mechanics 2ed J.J. Sakurai and J.J.
    Napolitano.
\end{thebibliography}
\printnomenclature
\section{License}
