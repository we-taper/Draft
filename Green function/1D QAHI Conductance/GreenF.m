...
w=1.*10^-3;                     % the resolution of energy

I=eye(4);
gL=g11(E,muL,ts,tsoL,w);   
Tc1=V_dl(tc1);        
...
Gnn=I/(((E+1i*w)*I-HeS1));      %| initial values of the first recursion, i.e. the isolated GF for the first component
G1n=Gnn;                        %|
Gn1=Gnn;                        %|
V1=V_sample(ts,tso);
V2=V1';
for i=2:L-1
    %i
    gnn=I/(((E+1i*w)*I-HnS+dis(i)*diag([-1,-1,1,1]))); % isolated GF for 2 ~ N-1 componets
	Gnn=(I-gnn*V2*Gnn*V1)\gnn;
	Gn1=Gnn*V2*Gn1;
	G1n=G1n*V1*Gnn;    
end

gnn=I/(((E+1i*w)*I-HeS2));      % isolated GF for the last component at lead-2
GNN=(I-gnn*V2*Gnn*V1)\gnn;
GN1=GNN*V2*Gn1;
G1N=G1n*V1*GNN; 

%% from right to left
Gnn=I/(((E+1i*w)*I-HeS2));      %| initial values of the first recursion, i.e. the isolated GF for the first component
GNn=Gnn;                        %|
GnN=Gnn;                        %|
for i=1:L-2
	%i
    gnn=I/(((E+1i*w)*I-HnS+dis(L-i)*diag([-1,-1,1,1]))); % isolated GF for 2 ~ N-1 componets
	Gnn=(I-gnn*V1*Gnn*V2)\gnn;
	GNn=GNn*V2*Gnn;
	GnN=Gnn*V1*GnN;    
end
gnn=I/(((E+1i*w)*I-HeS1));      % isolated GF for the last component
