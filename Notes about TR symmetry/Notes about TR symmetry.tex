...
\end{enumerate}
    ...
    equality (let $\Theta$ dentoes the time reversal operator):
    \begin{align}
        \left(1-\frac{iH}{\hbar}\delta t\right) \Theta \ket{\alpha}
        = \Theta\left(1-\frac{iH}{\hbar}(-\delta t)\right)\ket{\alpha}
    ...
    One can show that \textbf{$\Theta$ must be antilinear}. Because if one
    assumes that $\Theta$ is linear, one will find that the energy
    ...
    So we have:
    \begin{itemize}
        \item $\Theta$ is antiunitary.
        \item $\Theta H=H\Theta$.
    \end{itemize}

    
    Recall that $\Theta= UK$, and $K$ is highly dependent on the basis
    chosen. So $U$ will depend on the basis too. To get $\Theta$ in a
    ...
    \end{align}
    where $\ket{}$ stands for any ket. Then clearly, $\Theta$ is just
    complex conjugation for wave function $\Psi(x)$. But $\Theta$ acts on
    ...
    \subsection{Time reversal operator in spin \texorpdfstring{$j$}{}}
    ...
    \begin{align}
        ...
        \Theta^2 &= -1 \text{ if $j$ is a half-integer} \\
        \Theta^2 &= 1 \text{ if $j$ is an integer}
    ...
    \begin{proof}
        ...
        reversal symmetric, i.e. $[H,\Theta]=0$, then $\Theta \ket{n}$ is
        another energy eigenstate with the same energy. Supposing there is
        no degeneracy, then we have
        $$ \ket{n} = e^{i\eta}\Theta\ket{n}$$
        for some $\eta\in \mathbb{R}$. Apply $\Theta$ on both sides, then
        we have
        \begin{align*}
            e^{-i\eta} \Theta^2 \ket{n} &= \pm e^{-i\eta}\ket{n} \\
            &= \Theta \ket{n} = e^{i\eta} \ket{n}
        \end{align*}
        Clearly this is a contracdiction unless $\Theta^2=1$, which is
    ...
    to be shown here.
    But it shows an interesting fact that $\Theta^2=-1$, actually
    ...
    $\bra{\Theta \psi}H\ket{\psi}$ from a state to its time reversed
    state.

    \begin{align*}
        & \braket{\Theta\psi|H|\psi} = (U_{mp}K\psi_p)^*H_{mn}\psi_n 
        =U_{mp}^* \psi_p H_{mn} \psi_n = (U^\dagger)_pm\psi_p H_{mn}\psi_n
        = (U^\dagger)_{pm}\psi_p (THT^{-1})_{mn} \psi_n
        \\
        &= (U^\dagger)_{pm}\psi_p \left(U_{mr}KH_{rq}(-U_{qn}K)\right)\psi_n\\
        &= - (U^\dagger)_{pm}\psi_p \left(U_{mr}H^*_{rq}U^*_{qn}\right)\psi_n\\
        &= - \psi_p \left((U^\dagger)_{pm}
            U_{mr}\right)H^*_{rq}U^*_{qn}\psi_n\\
        &= - \psi_p H^*_{pq}U^*_{qn}\psi_n\\
            \overset{\text{by ...}}{=}& - \psi_p U^*_{pq}H_{qn}\psi_n \\
        &= U^*_{qp}\psi_p H_{qn} \psi_n
    \end{align*}
    Either there is a mistake in Bernevig's derivation or I am wrong...
\section{Anchor}
\label{sec:Anchor}
